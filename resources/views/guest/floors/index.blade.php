            ...
            <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center">
                    <div class="inline-block mb-6">
                        <span class="text-sm font-semibold text-green-600 uppercase tracking-wider bg-green-50 px-4 py-2 rounded-full">Directory</span>
                    </div>
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
            ...
            @if($floorsWithImages->count() > 0)
                ...
                <div class="mb-20">
                    <div class="text-center mb-12">
                        <h2 class="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4">Building Floor Plans</h2>
                        <p class="text-gray-600 text-lg md:text-xl">Swipe or use navigation to explore each floor</p>
                    </div>

                    <!-- Carousel Container -->
                    <div class="relative bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-gray-200/50" x-data="floorCarousel()">
                        <!-- Main Image Display -->
                        ...
                        <!-- Floor Thumbnails -->
                        <div class="p-6 md:p-8 bg-gradient-to-br from-gray-50/80 to-blue-50/80 backdrop-blur-sm">
                            <div class="flex space-x-4 overflow-x-auto pb-2 scrollbar-hide">
                                @foreach($floorsWithImages as $index => $floor)
                                    <button @click="goToFloor({{ $index }})" 
                                            ...
                                            :class="currentIndex === {{ $index }} ? 'border-blue-500 ring-4 ring-blue-200 shadow-xl scale-110' : 'border-gray-200 hover:border-blue-300 shadow-lg'">
                                        <img src="{{ asset('images/floors/' . $floor->image) }}" 
                                             alt="{{ $floor->name }}" 
                                             class="w-full h-full object-cover">
                                    </button>
                        ...
                        <div class="group relative bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500 border border-gray-200/50 transform hover:-translate-y-4 hover:scale-[1.02]">
                            <div class="relative overflow-hidden">
                                ...
                                @if($floor->level)
                                    <div class="absolute top-4 right-4">
                                        <span class="px-4 py-2 bg-white/90 backdrop-blur-xl text-blue-800 text-sm font-bold rounded-full shadow-xl border border-white/50">
                                            Level {{ $floor->level }}
                                        </span>
                                    </div>
                                @endif
                            ...
                            <div class="p-6 md:p-8">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="text-2xl md:text-3xl font-extrabold text-gray-900 group-hover:text-green-600 transition-colors duration-300">{{ $floor->name }}</h3>
                                </div>
                                
                                @if($floor->description)
                                    <p class="text-gray-600 mb-6 leading-relaxed text-base">{{ Str::limit($floor->description, 120) }}</p>
                                @endif
            ...
            @else
                <div class="text-center py-20 md:py-32">
                    ...
                    <h3 class="text-3xl md:text-4xl font-extrabold text-gray-900 mb-4">No floors available</h3>
                    <p class="text-gray-600 text-lg md:text-xl mb-10 max-w-2xl mx-auto">Floor directory information is not available at the moment.</p>
