...
@section('header')
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-3xl font-bold text-gray-900">Floor Directory Management</h2>
            <p class="mt-2 text-gray-600">Manage library floors, facilities, and floor plans</p>
        </div>
...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 border-l-4 border-blue-500">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600">Active Floors</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $floors->where('is_active', true)->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 border-l-4 border-purple-500">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $floors->where('image', '!=', null)->count() }}</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-200">
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <span class="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">Order: {{ $floor->sort_order }}</span>
                        </div>
                        ...
                        @if($floor->description)
                            <p class="text-sm text-gray-600 mb-4 line-clamp-2">{{ Str::limit($floor->description, 120) }}</p>
                        @endif
                        
                        @if($floor->facilities && count($floor->facilities) > 0)
                            <div class="mb-4">
                                <p class="text-xs font-medium text-gray-500 mb-2">Facilities:</p>
                                <div class="flex flex-wrap gap-1">
                                    ...
                                    @if(count($floor->facilities) > 3)
                                        <span class="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">+{{ count($floor->facilities) - 3 }} more</span>
                                    @endif
                                </div>
                            </div>
                        @endif
    ...
    @else
        <div class="bg-white rounded-lg shadow-lg p-12 text-center">
            ...
            <p class="text-gray-500 mb-6">Get started by creating your first floor directory entry.</p>
            <a href="{{ route('admin.floors.create') }}" 
               class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white font-medium rounded-lg hover:from-green-700 hover:to-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200 shadow-lg hover:shadow-xl">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
