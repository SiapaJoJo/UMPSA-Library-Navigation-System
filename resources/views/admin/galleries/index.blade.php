...
@section('header')
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-3xl font-bold text-gray-900 dark:text-white">Gallery Management</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 border-l-4 border-pink-500">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-300">Total Images</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $galleries->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 border-l-4 border-yellow-500">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-300">Featured Images</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $galleries->where('is_featured', true)->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 border-l-4 border-green-500">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-300">Active Images</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $galleries->where('is_active', true)->count() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    @if($galleries->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach($galleries as $gallery)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-200 group">
                    ...
                    <div class="p-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2 line-clamp-1">{{ $gallery->title }}</h3>
                        
                        @if($gallery->category)
                            <p class="text-sm text-pink-600 dark:text-pink-400 font-medium mb-2">
                        ...
                        <div class="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-gray-700">
                            <div class="flex space-x-2">
                                <a href="{{ route('admin.galleries.show', $gallery) }}" 
                                   class="inline-flex items-center text-pink-600 dark:text-pink-400 hover:text-pink-800 dark:hover:text-pink-300 text-sm font-medium transition-colors">
                            ...
                            <div class="flex items-center space-x-2">
                                <span class="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full">#{{ $gallery->sort_order }}</span>
